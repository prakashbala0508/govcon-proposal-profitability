...
fringe_base   = float(indirect[indirect["rate_type"] == "Fringe"]["base_rate"])
overhead_base = float(indirect[indirect["rate_type"] == "Overhead"]["base_rate"])
ga_base       = float(indirect[indirect["rate_type"] == "G&A"]["base_rate"])
...
if module == "Proposal Cost Build":
    ...
    fee_rates = {"Cost-Plus-Fixed-Fee (CPFF)": 0.07, "Time and Materials (T&M)": 0.10, "Firm-Fixed-Price (FFP)": 0.12}
    fee_rows = []
...
# ===================================================
    ...
    summary["fee_at_risk"] = summary.apply(
        lambda r: max(0,(r["total_cost"]/r["contract_ceiling"]-0.90)*r["contract_ceiling"]*r["fee_rate"]), axis=1
    )
    ...
    disp = summary[["award_id","contract_name","contract_type",
                    "total_cost","total_revenue","total_margin","margin_pct","fee_at_risk"]].copy()
    ...
    if selected == "All Contracts":
        plot_data = profitability.groupby("month")[["cost_incurred","revenue","gross_margin"]].sum().reset_index()
    ...
    st.markdown(
        f"Portfolio blended margin is **{summary['total_margin'].sum()/summary['total_revenue'].sum()*100:.1f}%** "
...
# ===================================================
    ...
    synth_data = pd.DataFrame([
        {"Data Element": "Monthly cost actuals",     "Why Synthetic": "Internal contractor records — never publicly disclosed"},
        {"Data Element": "Revenue by period",         "Why Synthetic": "Derived from synthetic cost actuals and fee rates"},
        {"Data Element": "Gross margin by month",     "Why Synthetic": "Derived from synthetic revenue and cost"},
        {"Data Element": "Labor hours by category",  "Why Synthetic": "Internal timesheet data — proprietary"},
        {"Data Element": "Proposal labor inputs",    "Why Synthetic": "Pre-award cost estimates — proprietary"},
    ])
